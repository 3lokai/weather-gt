...
`;
  ...
  useEffect(() => {
    ...
    // Create new palette texture with updated colors
    function makePaletteTexture(stops: string[]): THREE.DataTexture {
      let arr: string[];
      if (Array.isArray(stops) && stops.length > 0) {
        arr = stops.length === 1 ? [stops[0], stops[0]] : stops;
    ...
    // Update the shader material with new palette texture
    const material = (webgl.output as any).output.material as THREE.RawShaderMaterial;
